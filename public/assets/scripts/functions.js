...
function createIncrementControlHere(opts)
{
    insertHere(createIncrementControl(opts));
}

function createIncrementControl(opts)
{
    combineWithDefaults(opts,
    {
        min: 1,
    ...
    var currentValue = opts.initialValue;

    var container = createElement("div");
    container.classList.add("increment-control");

    var labelDiv = createAndAppend("div", container);
    labelDiv.classList.add("control-label");
    labelDiv.innerHTML = opts.label;

    var buttonsDiv = createAndAppend("div", container);
    buttonsDiv.classList.add("control-buttons");

    var minusBtn = createAndAppend("button", buttonsDiv);
    minusBtn.innerHTML = "−";
    minusBtn.classList.add("minus");

    var plusBtn = createAndAppend("button", buttonsDiv);
    plusBtn.innerHTML = "+";
    plusBtn.classList.add("plus");

    var updateValue = function(delta) {
        currentValue = clamp(opts.min, currentValue + delta, opts.max);
        opts.update(currentValue);
    };

    minusBtn.addEventListener("click", function() { updateValue(-opts.step); });
    plusBtn.addEventListener("click", function() { updateValue(opts.step); });
...
function createTimeSeries(opts)
{
    ...
    if (timeSeries.title) {
        ...
        titleDiv.innerText = timeSeries.title;
        timeSeries.div.appendChild(titleDiv);
    }
